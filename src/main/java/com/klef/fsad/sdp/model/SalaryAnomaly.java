...
public class SalaryAnomaly {
    ...
    @JsonIgnoreProperties({"anomalies", "employee"})
    private Salary salary;

    @ManyToOne
    @JoinColumn(name = "emp_id", nullable = false)
    @JsonIgnoreProperties({"attendances", "hr", "password", "resetToken", "tokenExpiryDate"})
    ...
    private void determineSeverity() {
        ...
        if (absDeviation >= 50) this.severity = "CRITICAL";
        else if (absDeviation >= 30) this.severity = "HIGH";
        else if (absDeviation >= 20) this.severity = "MEDIUM";
