...
import com.fasterxml.jackson.annotation.JsonIgnore; // Recommended to avoid infinite loops
...
public class Salary {
    ...
    // --- NEW: Add this to fix the Delete Error ---
    // This tells Hibernate: If I delete a Salary, delete all linked anomalies too.
    @OneToMany(mappedBy = "salary", cascade = CascadeType.ALL, orphanRemoval = true)
    @JsonIgnore // Prevents JSON recursion during API calls
    private List<SalaryAnomaly> anomalies;
    ...
    // --- Added Getter and Setter for Anomalies ---
    public List<SalaryAnomaly> getAnomalies() {
        return anomalies;
    }

    public void setAnomalies(List<SalaryAnomaly> anomalies) {
